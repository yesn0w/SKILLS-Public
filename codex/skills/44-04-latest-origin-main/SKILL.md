...
- When running inside this skills repository, if the fast-forward updates files under `codex/skills/44-04-latest-origin-main/`, sync the local installed Codex skill after the repository is clean. Treat symlink installs as already synced only when they resolve to the updated source; for copy installs, preserve a backup before mirroring the package.
...
9. If the current repository contains `codex/skills/44-04-latest-origin-main/`, check whether that package changed between the previous local `main` commit and the updated `main` commit:
   - compare with `git diff --name-only <previous-main> <updated-main> -- codex/skills/44-04-latest-origin-main`
   - if files changed, sync `${CODEX_SKILLS_DIR:-${CODEX_HOME:-$HOME/.codex}/skills}/44-04-latest-origin-main`
...
```bash
bash ~/.codex/skills/44-04-latest-origin-main/scripts/go_to_latest_origin_main.sh
```
...
- Local skill sync status when this repository updates files under `codex/skills/44-04-latest-origin-main/`, including changed skill files and any backup path.
